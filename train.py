...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    torch.save(model.state_dict(), 'logs/Centernet/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
...
if __name__ == "__main__":
    ...
    #   权值文件请看README，百度网盘下载
    #------------------------------------------------------#
    if os.path.exists(f"model_data/Centernet_{str(edge)}.pth"):
        model_path = f"model_data/Centernet_{str(edge)}.pth"
    ...
    torch.save(obj=model.state_dict(),f=f'model_data/Centernet_{str(edge)}.pth')
