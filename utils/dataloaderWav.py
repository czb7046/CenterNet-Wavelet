...
class CenternetDataset(Dataset):
    def __init__(self, train_lines, input_size, num_classes, is_train, edge):
    ...
    def __getitem__(self, index):
        ...
        CA1, HVD2, HVD3, HVD4 = self.waveDecom2(img,'db1')

        return CA1, HVD2, HVD3, HVD4, batch_hm, batch_wh, batch_reg, batch_reg_mask


# DataLoader中collate_fn使用
def centernet_dataset_collate(batch):
    CA1s, HVD2s, HVD3s, HVD4s, batch_hms, batch_whs, batch_regs, batch_reg_masks = [], [], [], [], [], [], [], []

    for CA1, HVD2, HVD3, HVD4, batch_hm, batch_wh, batch_reg, batch_reg_mask in batch:
        # imgs.append(img)
        CA1s.append(CA1)
        HVD2s.append(HVD2)
        HVD3s.append(HVD3)
    ...
    # imgs = np.array(imgs)
    CA1s =np.array(CA1s)
    HVD2s = np.array(HVD2s)
    HVD3s =np.array(HVD3s)
    HVD4s = np.array(HVD4s)
    ...
    return CA1s, HVD2s, HVD3s, HVD4s, batch_hms, batch_whs, batch_regs, batch_reg_masks
