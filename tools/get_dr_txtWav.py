...
from tools.centernetWav import CenterNet
...
此处获得的./inputWav/detection-results/里面的txt的框的数量会比直接predict多一些，这是因为这里的门限低，
...
class mAP_CenterNet(CenterNet):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self,image_id,image,fileresults):
        f = open(fileresults+"/"+image_id+".txt","w")
        ...
        images = [np.reshape(images[i], (1, images[i].shape[0], images[i].shape[1], images[i].shape[2])) for i in
                  range(len(images))]
