...
from utils.dataloaderWav import CenternetDataset, centernet_dataset_collate
...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            CA1s, HVD2s, HVD3s, HVD4s, batch_hms, batch_whs, batch_regs, batch_reg_masks = batch
            batch_images = [CA1s, HVD2s, HVD3s, HVD4s]
            optimizer.zero_grad()
...
def get_Pth_File_Path(year,edge):
    if year == '2020':
        filepath = f"model_data/CenterNet_Wav_{str(edge)}.pth"
    elif year =='2007':
        filepath = f"model_data/CenterNet_Wav_voc_{str(edge)}.pth"
    else:
        filepath = f"model_data/CenterNet_Wav_{str(edge)}.pth"
    return filepath
if __name__ == "__main__":
    ...
    parser.add_argument('--fep', type=int)
    parser.add_argument('--ufep', type=int)
    parser.add_argument('--classespath',type=str,default='model_data/imo_classes.txt')
    parser.add_argument('--ann_year',type=str,default='2020')
    args = parser.parse_args()
    if args.ann_year == '2020':
       assert args.ann_year == '2020' and args.classespath == 'model_data/imo_classes.txt'
    if args.ann_year == '2007':
       assert args.ann_year == '2007' and args.classespath == 'model_data/voc_classes.txt'
    #-------------------------------------------#
    #   输入图片的大小
    #-------------------------------------------#
    # input_shape = (512,512,3)
    edge=args.surfix
    input_shape = (edge, edge, 3)
    #-----------------------------#
    #   训练前一定要注意注意修改
    #   classes_path对应的txt的内容
    #   修改成自己需要分的类
    #-----------------------------#
    classes_path = args.classespath
    #----------------------------------------------------#
    #   获取classes和数量
    #----------------------------------------------------#
    ...
    #-------------------------------------------#
    #   主干特征提取网络的选择
    #   resnet50和hourglass和resnetWav50
    #-------------------------------------------#
    backbone = "resnet50"
    #-------------------------------#
    #   是否使用Cuda
    ...
    #   权值文件请看README，百度网盘下载
    #------------------------------------------------------#
    filepath = get_Pth_File_Path(args.ann_year,edge)
    ...
    #   获得图片路径和标签
    #----------------------------------------------------#
    # strPth=os.path.dirname(__file__)
    annotation_path = f'{args.ann_year}_train.txt'
    #----------------------------------------------------------------------#
    ...
    if True:
        ...
        Batch_size = 48
        Init_Epoch = 0
        Freeze_Epoch = args.fep
        
        optimizer = optim.Adam(net.parameters(),lr,weight_decay=5e-5)
        lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=2, verbose=True)

        train_dataset = CenternetDataset(lines[:num_train], input_shape, num_classes, True, edge)
        val_dataset = CenternetDataset(lines[num_train:], input_shape, num_classes,  False, edge)
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch,Unfreeze_Epoch):
            ...
            lr_scheduler.step(val_loss)
    torch.save(obj=model.state_dict(),f=filepath)
